...
@interface CustomStyleChartVC ()<AAChartViewEventDelegate>
...
- (void)viewDidLoad {
    ...
    [self.aaChartView addSubview:self.markButton];
}
...
//    CGRect chartViewFrame = CGRectMake(0, 58, self.view.frame.size.width, self.view.frame.size.height - 88);
    CGRect chartViewFrame = CGRectMake(0, 200, self.view.frame.size.width, 125.0f);
...
- (AAChartModel *)configureGradientColorAreasplineChart {
    ...
    [AAGradientColor gradientColorWithDirection:AALinearGradientDirectionToBottom
                               startColorString:@"rgba(130,160,246,0.8)"//深粉色, alpha 透明度 1
                                 endColorString:@"rgba(130,160,246,0.1)"];//热情的粉红, alpha 透明度 0.3 背景填充的颜色
...
//    .categoriesSet(@[@"一月", @"二月", @"三月", @"四月", @"五月", @"六月",
//                     @"七月", @"八月", @"九月", @"十月", @"十一月", @"十二月"])
    .xAxisVisibleSet(false)//不显示X轴
    .yAxisVisibleSet(false)//不显示Y轴
    .yAxisTitleSet(@"")
    .touchEventEnabledSet(true)
    .markerRadiusSet(@0)//marker点半径为8个像素
    .markerSymbolStyleSet(AAChartSymbolStyleTypeBorderBlank)//marker点为实心
    .markerSymbolSet(AAChartSymbolTypeCircle)//marker点为圆形点○
    .yAxisLineWidthSet(@0)
    .yAxisGridLineWidthSet(@0)
    .legendEnabledSet(false)//图例,最底部的东西
    .easyGradientColorsSet(true)
    .tooltipEnabledSet(false)//不显示标注

    .seriesSet(@[
        AASeriesElement.new
        .nameSet(@"Tokyo Hot")
        .lineWidthSet(@3.0)
        .colorSet(@"rgba(130,160,246,1)")//猩红色, alpha 透明度 1
        .fillColorSet((id)gradientColorDic1)
        .dataSet(@[@15.0, @20.0, @12.0, @15.0, @20.0, @15.0]),
    ]);
}

#define MarkBtnWeight 15.0f
...
- (void)aaChartView:(AAChartView *)aaChartView moveOverEventWithMessage:(AAMoveOverEventMessageModel *)message {
    ...
    //这里可以实现监听里面的X轴和Y轴的坐标,实时创建一个全局Btn,然后切换到对应的位置显示;
    [self.markButton setFrame:CGRectMake([message.offset[@"plotX"] floatValue]+2.0f, [message.offset[@"plotY"] floatValue]+2.0f, MarkBtnWeight, MarkBtnWeight)];
...
{
    if (!_markButton) {
        _markButton = [[UIButton alloc]initWithFrame:CGRectMake(100, 600,MarkBtnWeight,MarkBtnWeight)];
        _markButton.backgroundColor = [UIColor colorWithRed:130/255.0 green:140/255.0 blue:246/255.0 alpha:1.0];
        _markButton.layer.masksToBounds = YES;
        _markButton.layer.cornerRadius = MarkBtnWeight/2.0f;
        _markButton.layer.borderWidth = 2;
        _markButton.layer.borderColor = [[UIColor whiteColor] CGColor];
